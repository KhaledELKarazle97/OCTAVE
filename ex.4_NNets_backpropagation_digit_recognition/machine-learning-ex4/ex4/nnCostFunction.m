...
A1 = [ones(m, 1) X]; %adds a column of ones to X and calls it A1 (input layer)
Z2 = A1 * Theta1'; %takes a1 linearly to z2 (second or hidden layer)
A2 = [ones(size(Z2, 1), 1) sigmoid(Z2)]; %sigmoid activation function on hidden layer to get A2
Z3 = A2 * Theta2'; %multiplying by theta2 brings A2 to Z3 in output layear of layer 3
